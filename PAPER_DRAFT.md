...
Reliable sender generation is not established. A neutral-ID Capsule-guided pilot selects all 16 emit-or-reject decisions and all ten acts correctly, but only six of ten generated messages pass structural validation and preserve the essential semantics. The standardized Teachability Score remains unset because exact target graphs, frame parsing, unseen-partner cross-play, sample efficiency, and the Capsule's full safety gates are not measured. An earlier 36/36 smoke pilot exposed substantial task cues and is retained only as open-label evidence.
