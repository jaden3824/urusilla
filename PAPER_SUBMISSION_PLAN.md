...
| Receiver path | Complete and test the same-context Capsule-to-direct-action-state executor with bound raw/JSON fallback. Reject malformed payloads before the receiver call. | Run provider-neutral integration pilots; demonstrate that task-critical mutation changes the result and irrelevant re-encoding does not, without counting pilots as confirmation. | Freeze the exact runtime revision and request artifacts used for confirmation. | Required for AAMAS go; if absent or fake-adapter-only, AAMAS is no-go and TMLR work continues without a causal-use claim. |
