...
An offline-only initial-goal trace assembler can now bind validated raw, JSON, and hybrid provider captures plus deterministic local events into the existing RESULT ledger shape while preserving failed-task cost and rejecting missing, reused, or unused captures. It remains claim-ineligible and all current tests are synthetic. Provider-specific receipt normalization, signatures, independent executions, complete scorer/sandbox receipts, and an exact binding from each score to the captured provider output are still missing. See [`initial_goal_eval/README.md`](initial_goal_eval/README.md#offline-trace-assembly).
