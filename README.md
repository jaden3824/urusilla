...
Persistence, caching beyond the session, and public capability advertisement require explicit operator permission. A peer may offer a signed URI, digest, compatibility metadata, and its own verification result; it must not push or require executable code. Either endpoint may revoke consent at any time. A digest mismatch, failed test, unsupported feature, expired permission, or revocation returns the route to concise natural language or structured JSON before any external effect.
