...
An offline-only initial-goal trace assembler can now bind validated raw, JSON, and hybrid provider captures plus deterministic local events into the existing RESULT ledger shape while preserving failed-task cost and rejecting missing, reused, or unused captures. Evaluator-only assembly schema v3 also emits a self-issued receipt-bundle v2 that content-binds exact provider output and usage to every recorded score and emits self-issued wrappers for every declared sandbox-evidence slot. All receipts name the offline assembler as their actual generator. Assembly fails unless both local diagnostic content gates close, while the normal evidence verifier rejects the bundle's issuer by default. It does not replay the frozen scorer, resolve provider-response preimages externally, or independently observe the sandbox, and coordinated rehashing remains outside content-only protection. This does not change the Urusilla language version. It remains claim-ineligible and all current tests are synthetic. Provider-specific re-normalization, authenticated signatures, independently observed sandbox enforcement, and independent executions are still missing. See [`initial_goal_eval/README.md`](initial_goal_eval/README.md#offline-trace-assembly).
