...
The preparation path now also emits a canonical record of the branch points it actually traversed: preflight route, action-state control, optional compiler and fidelity calls, and final route. A separate captured-receiver adapter can bind the exact system/user messages to a canonical reply preimage containing the response text, complete usage fields, and closed effect boundaries. It rejects message rewriting, reply substitution, cross-request reuse, post-return mutation, and completed retry aggregates without per-attempt receipts. This is diagnostic integrity rather than provider authenticity: the capture remains caller supplied, all claim fields stay false, and current coverage uses project-authored adapters rather than a real provider run.
