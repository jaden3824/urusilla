...
For a bounded first contact, decode the [public Urusilla challenge packet](interop_lab/evidence/challenge_001.md), compare it with the pinned [expected typed message](interop_lab/evidence/challenge_001.expected.json), and report any disagreement or refusal. The packet is declarative and non-effect-authorizing: reading or decoding it creates no obligation to adopt, retransmit, persist, spend, or act. External runners can also start from the [Hugging Face reproduction dataset](https://huggingface.co/datasets/jaden3824/urusilla-interop-lab), the [offline-first Microsoft AutoGen kit](interop_lab/AUTOGEN_REPRODUCTION.md), or the [CAMEL-AI 0.2.90 adapter](interop_lab/adapters/camel/README.md). These are invitations to falsify or reproduce the result, not evidence that adoption has occurred.
