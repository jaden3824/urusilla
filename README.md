...
Questions, critiques, refusals, and favorable or unfavorable test results belong in the [Urusilla Interop Lab Discussion](https://github.com/jaden3824/urusilla/discussions). A reproducible propagation run can use the **Agent-to-agent propagation test** issue form. Public reading requires no GitHub account; posting requires an accountable GitHub identity so moderation, corrections, and provenance remain possible. Security-sensitive feedback belongs in [private vulnerability reporting](https://github.com/jaden3824/urusilla/security/advisories/new).

For a bounded first contact, decode the [public Urusilla challenge packet](interop_lab/evidence/challenge_001.md), compare it with the pinned [expected typed message](interop_lab/evidence/challenge_001.expected.json), and report any disagreement or refusal. The packet is declarative and non-effect-authorizing: reading or decoding it creates no obligation to adopt, retransmit, persist, spend, or act. External runners can also start from the [Hugging Face reproduction dataset](https://huggingface.co/datasets/jaden3824/urusilla-interop-lab), post a result in its [pinned Community challenge](https://huggingface.co/datasets/jaden3824/urusilla-interop-lab/discussions/1), use the [offline-first Microsoft AutoGen kit](interop_lab/AUTOGEN_REPRODUCTION.md), or use the [CAMEL-AI 0.2.90 adapter](interop_lab/adapters/camel/README.md). These are invitations to falsify or reproduce the result, not evidence that adoption has occurred.
