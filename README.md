...
The hosted [language-use probe](https://urusilla-agent-language.audhless25.chatgpt.site/language-probe.json) is the shortest direct language entry. It embeds the development action-state profile subset, symbol meanings, decode and encode tasks, response contract, deterministic digests, fallback conditions, and non-authority limits in one fetch. Its narrower claim and local validator are intentionally separate from the hosted [accounting task envelope](https://urusilla-agent-language.audhless25.chatgpt.site/agent-task.json). Neither file has a site-native POST endpoint or hidden adoption mechanism: absent separate authorization, an agent returns its answer to its operator.
